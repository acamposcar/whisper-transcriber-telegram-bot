...
restart_on_failure = config.getboolean(
    "GeneralSettings", "RestartOnConnectionFailure", fallback=True
)
...
class TranscriberBot:
    ...
    def __init__(self):
        ...
        self.is_processing = (
            asyncio.Lock()
        )  # Lock to ensure one transcription at a time

        self.restart_on_failure = (
            restart_on_failure  # Controls the restart behavior on connection failure
        )
        ...
        self.cooldown_seconds = self.config.getint(
            "RateLimitSettings", "cooldown_seconds", fallback=10
        )
        self.max_requests_per_minute = self.config.getint(
            "RateLimitSettings", "max_requests_per_minute", fallback=5
        )
        ...
        self.allowed_formats = self.config.get(
            "AllowedFileFormats", "allowed_formats", fallback="mp3, wav, mp4"
        ).split(",")
        self.allowed_formats = [fmt.lower().strip() for fmt in self.allowed_formats]

        # Allow video files (true/false)
        self.allow_video_files = self.config.getboolean(
            "AudioSettings", "allowvideofiles", fallback=False
        )
        ...
        self.valid_models = self.config.get(
            "ModelSettings",
            "ValidModels",
            fallback="tiny, base, small, medium, large, turbo",
        ).split(", ")

        self.transcription_settings = {
            "includeheaderintranscription": self.config.getboolean(
                "TranscriptionSettings", "includeheaderintranscription", fallback=True
            ),
            "keepaudiofiles": self.config.getboolean(
                "TranscriptionSettings", "keepaudiofiles", fallback=False
            ),
            "sendasfiles": self.config.getboolean(
                "TranscriptionSettings", "sendasfiles", fallback=True
            ),
            "sendasmessages": self.config.getboolean(
                "TranscriptionSettings", "sendasmessages", fallback=False
            ),
        }
        ...
        self.user_models_lock = (
            asyncio.Lock()
        )  # Lock for handling user_models dictionary

        # Read the maximum file size setting
        self.max_file_size_mb = self.config.getint(
            "AudioSettings", "max_file_size_mb", fallback=20
        )
    ...
    async def handle_message(self, update: Update, context: CallbackContext) -> None:
        ...
        if (now - last_request_time).seconds < self.cooldown_seconds:
            await update.message.reply_text(
                f"Please wait {self.cooldown_seconds - (now - last_request_time).seconds} seconds before making another request."
            )
            return
        ...
        if last_request_time < minute_ago:
            self.user_request_counts[user_id] = (
                0  # Reset the count if the last request was over a minute ago
            )
        ...
        logger.debug(
            f"Current model for user {user_id} before transcription: {current_model}"
        )
        ...
        if update.message and update.message.text:
            ...
            if urls:
                ...
                logger.info(
                    f"Task added to the queue. Current queue size: {queue_length}"
                )
                ...
                await update.message.reply_text(
                    "❌ No valid URL detected in your message. Please send a message that includes a valid URL. If you need help, type: /help"
                )
                ...
                try:
                    async with TranscriberBot.processing_lock:
                        ...
                        try:
                            ...
                            video_info_message = (
                                ""  # Set a default empty value for video_info_message
                            )
                            ai_transcript_header = (
                                ""  # Set a default empty value for ai_transcript_header
                            )
                            ...
                            if isinstance(task, str) and task.startswith("http"):
                                ...
                                await process_url_message(
                                    task, bot, update, model, language
                                )
                                success = (
                                    True  # Mark as successful if no exception occurs
                                )
                            elif any(
                                task.endswith(f".{ext}") for ext in self.allowed_formats
                            ):
                                ...
                                if best_gpu:
                                    ...
                                    device = "cpu"
                                    gpu_message = (
                                        "No GPU available, using CPU for transcription."
                                    )
                                ...
                                if audio_duration is None or audio_duration > 180:
                                    try:
                                        await bot.send_message(
                                            chat_id=update.effective_chat.id,
                                            text="Fichero de audio no valido o audio demasiado largo (3 min).",
                                        )
                                    except Exception as e:
                                        logger.error(
                                            f"Failed to send invalid audio file message: {e}"
                                        )
                                ...
                                estimated_minutes = (
                                    estimated_time / 60
                                )  # Convert to minutes for user-friendly display

                                current_time = datetime.now()
                                estimated_finish_time = current_time + timedelta(
                                    seconds=estimated_time
                                )

                                time_now_str = current_time.strftime(
                                    "%Y-%m-%d %H:%M:%S"
                                )
                                estimated_finish_time_str = (
                                    estimated_finish_time.strftime("%Y-%m-%d %H:%M:%S")
                                )

                                formatted_audio_duration = format_duration(
                                    audio_duration
                                )
                                language_setting = (
                                    language if language else "autodetection"
                                )
                                detailed_message = (
                                ...
                                # except Exception as e:
                                #     logger.error(
                                #         f"Failed to send detailed message: {e}"
                                #     )
                                ...
                                # )
                                transcription_paths, raw_content = (
                                    await transcribe_with_replicate(task)
                                )

                                logger.info(
                                    f"Transcription paths returned: {transcription_paths}"
                                )
                                ...
                                try:
                                    ...
                                    content = raw_content  # Add transcription note
                                    ...
                                    chunks = safe_split_message(
                                        content, max_length=max_message_length
                                    )
                                    ...
                                    for i, chunk in enumerate(chunks):
                                        try:
                                            ...
                                            logger.error(
                                                f"Error sending message chunk {i + 1}: {e}"
                                            )
                                            ...
                                            try:
                                                await bot.send_message(
                                                    chat_id=update.effective_chat.id,
                                                    text="Error.",
                                                    # text="An error occurred while sending a transcription chunk.",
                                                )
                                            except Exception as e:
                                                logger.error(
                                    ...
                                    logger.error(
                                        f"Error in sending plain text messages: {e}"
                                    )
                                    # Optionally, inform the user about the error
                                    try:
                                        await bot.send_message(
                                            chat_id=update.effective_chat.id,
                                            text="Error.",
                                            # text="An error occurred while preparing the transcription messages.",
                                        )
                                    except Exception as e:
                                        logger.error(
                                            f"Failed to send error message to user: {e}"
                                ...
                                try:
                                    ...
                                    logger.error(
                                        f"Failed to delete audio file {task}: {e}"
                                    )

                                for filename in os.listdir(audio_messages_dir):
                                    file_path = os.path.join(
                                        audio_messages_dir, filename
                                    )
                                    try:
                                        os.remove(file_path)
                                        ...
                                        logger.error(
                                            f"Failed to delete audio file {file_path}: {e}"
                                        )
                        except Exception as e:
                            logger.error(
                                f"An error occurred while processing the task: {e}"
                            )
                            # Optionally, inform the user about the error
                            try:
                                await bot.send_message(
                                ...
                                logger.error(
                                    f"Failed to send error message to user: {ex}"
                                )
                            ...
                            # if (
                            #     success
                            #     and notification_settings["send_completion_message"]
                            # ):
                            #     try:
                            #         completion_message = notification_settings[
                            #             "completion_message"
                            #         ]
                            #         await bot.send_message(
                            #             chat_id=update.effective_chat.id,
                            ...
                            #         )
                            #         logger.info(
                            #             f"Sent completion message to user ID {user_id}: {completion_message}"
                            #         )
                            #     except Exception as e:
                            #         logger.error(
                            #             f"Error sending completion message: {e}"
                            #         )
    ...
    async def set_language_command(
        ...
        supported_languages = self.config.get(
            "WhisperSettings", "supportedlanguages", fallback="auto"
        ).split(", ")
    ...
    async def help_command(self, update: Update, context: CallbackContext) -> None:
        models_list = ", ".join(
            self.valid_models
        )  # Dynamically generate the list of valid models
        allowed_formats_list = ", ".join(
            self.allowed_formats
        )  # Get the list of allowed formats

        # Access the settings
        allow_audio_files = self.config.getboolean(
            "AudioSettings", "allowaudiofiles", fallback=True
        )
        allow_voice_messages = self.config.getboolean(
            "AudioSettings", "allowvoicemessages", fallback=True
        )
        allow_video_files = self.config.getboolean(
            "AudioSettings", "allowvideofiles", fallback=False
        ...
        if allow_audio_files and allow_voice_messages and allow_video_files:
            ...
            file_upload_info = f"- Note: Direct file uploads and audio messages are currently disabled.\n"
    ...
    async def model_command(self, update: Update, context: CallbackContext) -> None:
        ...
        if new_model in self.valid_models:
            ...
            await update.message.reply_text(
                f"Model set to: <code>{new_model}</code>", parse_mode="HTML"
            )
        else:
            await update.message.reply_text(
    ...
    async def handle_audio_file(self, update: Update, context: CallbackContext) -> None:
        ...
        try:
            ...
            file_path = os.path.join(
                audio_messages_dir, f"{file_info.file_unique_id}.{file_extension}"
            )
    ...
    async def handle_video_file(self, update: Update, context: CallbackContext) -> None:
        ...
        try:
            ...
            video_file_path = os.path.join(
                self.video_messages_dir, f"{file.file_unique_id}.{file_extension}"
            )
            ...
            audio_file_path = os.path.join(
                self.audio_messages_dir, f"{file.file_unique_id}.mp3"
            )
            try:
                subprocess.run(
                    [
                ...
                await update.message.reply_text(response_text)
                logger.info(
                    f"Audio file queued for transcription. Queue length: {queue_length}"
                )

            except subprocess.CalledProcessError as e:
                logger.error(f"Error extracting audio from video file: {e}")
                await update.message.reply_text(
                    "An error occurred while extracting audio from the video."
                )
            finally:
            ...
            await update.message.reply_text(
                "An error occurred while processing your video file."
            )
    ...
    async def info_command(self, update: Update, context: CallbackContext) -> None:
        ...
        )  # Assuming self.start_time is set at bot launch
    ...
    def run(self):
        ...
        for sig in [signal.SIGINT, signal.SIGTERM]:
            loop.add_signal_handler(
                sig, lambda s=sig: asyncio.create_task(self.shutdown(s, loop))
            )
        ...
        while not connected:
            try:
                ...
                # self.application.add_handler(
                #     CommandHandler(["help", "about"], self.help_command)
                # )
                ...
                # )
                # self.application.add_handler(
                #     CommandHandler("language", self.set_language_command)
                # )
                ...
                self.application.add_handler(
                    MessageHandler(filters.Document.ALL, self.handle_audio_file)
                )

                # Add this line to handle video messages
                # self.application.add_handler(
                #     MessageHandler(filters.VIDEO, self.handle_video_file)
                # )
...
if __name__ == "__main__":
    ...
    check_for_updates = config.getboolean(
        "UpdateSettings", "CheckForYTDLPUpdates", fallback=False
    )
    update_command = config.get(
        "UpdateSettings", "UpdateCommand", fallback="pip install -U yt-dlp"
    )
